...
##### Getting the most contributing words for classifying the found module through a randomforest model

# Defining the training and the test datasets
f=merge(data.frame(cbind('paper_id'=rownames(modColors),modColors)),data.frame(cbind('paper_id'=rownames(m),data.frame(m))),by='paper_id')
rownames(f)=f$paper_id
head(f)
test.cases = c()
train.cases = c()
for(i in 1:length(unique(dynamicMods))){
	cl.id = which(f$dynamicMods==i)
	train = sample(cl.id,ceiling(0.8*length(cl.id)))
	test.id = which(!cl.id%in%train)
	test = cl.id[test.id]
	test.cases = append(test.cases, test)
	train.cases = append(train.cases, train)
	}
length(test.cases)
length(train.cases)

# Training the random forest model
rf = randomForest(dynamicMods~.,data=f[train.cases,c(2,4:ncol(f))], importance=TRUE, type='classification')
rf

# Out of bag perfomance 
rf$confusion

# Assesing the classification model against the test dataset
rf.t = predict(rf, f[test.cases,c(2,4:ncol(f))], type="response",norm.votes=TRUE, predict.all=FALSE, proximity=FALSE, nodes=FALSE)

perf = confusionMatrix(as.factor(rf.t), as.factor(f[test.cases,2]))
str(perf)
write.table(perf$table, "perf_confusionmatriz_rf_test.txt", sep="\t")
write.table(perf$byClass, "perf_infobyclass_rf_test.txt", sep="\t")
perf$overall
perf$byClass

# Assesing increasing number of best classificatory words
feat.selec = rfcv(f[train.cases,c(4:ncol(f))], f[train.cases,2])
pdf("feat_prior.pdf")
with(feat.selec, plot(n.var, error.cv, log="x", type="o",lwd=2))
dev.off()
str(feat.selec)
n.feat = which(feat.selec$error.cv<=0.30) # setting threshold
n.feat = min(as.numeric(names(n.feat)))

feat.imp = importance(rf)
class(feat.imp)
dim(feat.imp)
best.feat = head(feat.imp[order(feat.imp[,11],decreasing=T),],n.feat)
head(best.feat)
write.table(best.feat, "best_class_words.txt", sep="\t", quote=F, row.names=T)
...
for(color in colors){
    ...
    dtmrColor <- dtmr[rownames(colorMat), rownames(best.feat)[which(rownames(best.feat)%in%colnames(dtmr))]]
...
imp_dtmr <- dtmr[, rownames(best.feat)[which(rownames(best.feat)%in%colnames(dtmr))]]
